...
class MaskHead(pl.LightningModule):
    ...
    def __init__(self, Classes=3,P=14, batch_size = 4):
        ...
        self.pretrained_boxhead_path = 'model_trained_boxhead.pth'
        self.pretrained_fpn_path = 'checkpoint680.pth'
        self.model_boxhead = BoxHead()
        self.model_boxhead.load_state_dict(torch.load(self.pretrained_boxhead_path, map_location=device))
        self.model_boxhead = self.model_boxhead.to(device)
    ...
    def preprocess_ground_truth_creation(self, proposals, class_logits, box_regression, gt_labels,bbox ,masks , IOU_thresh=0.5, keep_num_preNMS=1000, keep_num_postNMS=10):
        ...
        for i, each_proposal in enumerate(proposals):
            ...
            if len(non_bg_label_idx) != 0: 
                ...
                above_thres_idx = iou_idx[:,0].to(device)
                above_thres_gt = iou_idx[:,1].to(device)
    ...
    def nms_preprocess_gt(self,clases,boxes,scores, masks, IOU_thres=0.5, keep_num_postNMS=10):
        ...
        if all(len(one_mask) == 0 for one_mask in masks_all):
            fin_masks = torch.empty(0, 800, 1088).to(device)
            fin_boxes = torch.empty(0, 4).to(device)
            fin_clas = torch.empty(0, 1).to(device)
            fin_scores = torch.empty(0, 1).to(device)
        else:
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.training_step_outputs.append({"loss":loss.detach().cpu().item()})
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_step_outputs.append({"loss":val_loss.detach().cpu().item()})
    ...
    def on_validation_epoch_end(self):
        ...
        for i in range(len(self.val_step_outputs)):
            avg_val_loss += self.val_step_outputs[i]["loss"]

        avg_val_loss /= len(self.val_step_outputs)
