...
from datasets.dataset import BuildDataset, BuildDataLoader
from pytorch_lightning.callbacks import ModelCheckpoint
from pytorch_lightning.callbacks import EarlyStopping
import pytorch_lightning.loggers as pl_loggers
from pytorch_lightning.loggers import tensorboard
from utils import MultiApply
import numpy as np
from utils import IOU, conv_box_to_xywh, IOU_gt, output_decoding, output_flattening
...
class RPNHead(pl.LightningModule):
    ...
    def __init__(self, device="cpu", batch_size = 4, num_anchors=3,
        ...
        self.resnet50_fpn = Resnet50Backbone(device=device)
    ...
    def create_ground_truth(self, bboxes, index, anchor_list, grid_size_list, image_size):
        ...
        for k in range(5):
            ...
            for i in range(self.num_anchors):
                ...
                remain_anchors = remain_anchors.to(device)

                all_ground_clas = torch.where(all_ground_clas[i] == 5., -1., all_ground_clas)
                all_ground_clas = all_ground_clas.to(device)

                iou_mat = IOU_gt(remain_anchors, bboxes).to(device)
            
                max_iou_each_bbox = torch.max(iou_mat, dim=0)[0]

                for j in range(bboxes.shape[0]):
                    pos_res_bools = torch.logical_or(iou_mat[:,j] >= 0.99*max_iou_each_bbox[j].item(), iou_mat[:,j] > 0.7).to(device)
                    pos_res_idx = torch.where(pos_res_bools == True)[0].to(device)
    ...
    def compute_loss(self, clas_out_list, regr_out_list, targ_clas_list, targ_regr_list, l=1.0, effective_batch=150):
    ...
    def on_train_epoch_end(self):
        ...
        avg_train_loss /= len(self.training_step_outputs)
        avg_class_loss /= len(self.training_step_outputs)
        avg_reg_loss /= len(self.training_step_outputs)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.val_step_outputs.append({"loss":val_loss.detach().cpu().item(), "classifier_loss":loss_class.detach().cpu().item(), "regressor_loss":loss_regr.detach().cpu().item()})
    ...
    def on_validation_epoch_end(self):
        ...
        for i in range(len(self.val_step_outputs)):
            avg_train_loss += self.val_step_outputs[i]["loss"]
            avg_class_loss += self.val_step_outputs[i]["classifier_loss"]
            avg_reg_loss += self.val_step_outputs[i]["regressor_loss"]

        avg_train_loss /= len(self.val_step_outputs)
        avg_class_loss /= len(self.val_step_outputs)
        avg_reg_loss /= len(self.val_step_outputs)
