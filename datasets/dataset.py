...
class BuildDataLoader(torch.utils.data.DataLoader):
    ...
    def loader(self):
        return DataLoader(self.dataset,
                          ...
                          collate_fn=self.collect_fn,drop_last=True)
...
if __name__ == '__main__':
    ...
    # rpn_net = RPNHead(device="cuda:1").to(device)
    # # push the randomized training data into the dataloader

    # # train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=0)
    # # test_loader = DataLoader(test_dataset, batch_size=2, shuffle=False, num_workers=0)
    # batch_size = 1
    # train_build_loader = BuildDataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=16)
    ...
    #     resnet50_fpn = Resnet50Backbone(device="cuda:1")
    #     backout = resnet50_fpn(images)

    #     fpn_feat_list = list(backout.values())

    #     plt.figure(figsize=(25,25))

    #     plt.subplot(1,len(fpn_feat_list)+1,1)
    #     plt.imshow(images[0].permute(1,2,0).cpu().numpy())
    #     plt.title("Original Image")
    #     plt.axis('off')

    #     for i in range(1,len(fpn_feat_list)):
    #         plt.subplot(1,len(fpn_feat_list)+1,i+1)
    #         plt.title("FPN"+str(i))
    #         plt.imshow(fpn_feat_list[i][:,0,:,:].detach().permute(1,2,0).cpu().numpy())
    #         plt.axis('off')
    ...
    #         plt.subplot(1,len(fpn_feat_list)+1,i+1)
    #         plt.title("FPN"+str(i))
    #         plt.imshow(rpn_net.intermediate_layer(fpn_feat_list[i])[:,1,:,:].detach().permute(1,2,0).cpu().numpy())
    #         plt.axis('off')

    #     plt.show()


    #     gt1,ground_coord1=rpn_net.create_batch_truth(boxes,indexes,images.shape[-2:])
    ...
    #     ax.imshow(images.squeeze(0).permute(1,2,0).cpu())
    ...
    #         col='r'
    #         #print(anchor)
    #         rect=patches.Rectangle((coord[0],coord[1]),coord[2]-coord[0],coord[3]-coord[1],fill=False,color=col) #type: ignore
    #         ax.add_patch(rect)
